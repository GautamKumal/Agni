...
const StudentsTable = () => {
  ...
  useEffect(() => {
    ...
    const fetchTeachersData = async () => {
      let { data, error } = await supabase.from("ManageStudents").select("*");
    ...
    if (error) {
      ...
      setStudentsTable(
        studentsTable.map((student) =>
          student.id === editRowId ? formData : student
        )
      );
      setEditRowId(null);
      setFormData({});
  ...
  const handleDelete = async (id) => {
    ...
    let { error } = await supabase.from("ManageTeachers").delete().eq("id", id);
